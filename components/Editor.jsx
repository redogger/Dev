...
const CLASSIC_THEME = {
  ...
  colors: {
    ...
    "editor.selectionBackground":     "#3169c455",
    "editorWidget.background":        "#d4d0c8",
    "editorWidget.border":            "#919b9c",
    "scrollbarSlider.background":     "#b8b4ac",
  },
};
...
export default function EditorPanel({ code, onChange }) {
  ...
  function beforeMount(monaco) {
    ...
    monaco.languages.registerCompletionItemProvider("cpp", {
      provideCompletionItems: (model, pos) => {
        ...
        return {
          suggestions: [
            { label:"cout", kind:14, detail:"Standard output", insertText:"cout << ", range },
            { label:"cin",  kind:14, detail:"Standard input",  insertText:"cin >> ",  range },
            { label:"endl", kind:14, detail:"End of line",     insertText:"endl",     range },
            { label:"string", kind:7, detail:"std::string", insertText:"string ", range },
            { label:"vector", kind:7, detail:"std::vector<T>", insertText:"vector<${1:int}> ${2:v};", insertTextRules:4, range },
  ...
  function onMount(editor, monaco) {
    ...
    editor.updateOptions({
      ...
      // ── Mobile fixes ──
      domReadOnly:              false,
      contextmenu:              !mobile,      // Disable native context menu on mobile
      quickSuggestions:         !mobile,      // Disable IntelliSense popup on mobile (blocks keyboard)
    ...
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: theme.colors.editor }}>
      ...
      >
        <div
          ...
          style={{
            background: theme.colors.tabActiveBg,
            borderBottom: theme.isClassic ? "1px solid #ece9d8" : `2px solid ${theme.colors.accent}`,
            borderRight: `1px solid ${theme.colors.border}`,
            color: theme.isClassic ? "#000" : theme.colors.accent,
            fontFamily: theme.isClassic ? "Tahoma, sans-serif" : theme.font,
            fontSize: theme.isClassic ? 11 : 11,
          }}
        >
          <span>📄</span><span>main.cpp</span>
        </div>
        <div style={{ flex: 1 }} />
        <div className="flex items-center px-4 gap-3 text-xs" style={{ color: theme.colors.label, fontFamily: "'Courier New', monospace", fontSize: 10 }}>
      ...
      <div className="flex-1 overflow-hidden">
        <MonacoEditor
          ...
          onMount={onMount}
          loading={<div style={{ display:"flex",alignItems:"center",justifyContent:"center",height:"100%",background:theme.colors.editor,color:theme.colors.accent,fontSize:12,fontFamily:"'Courier New',monospace" }}>Loading editor…</div>}
          options={{
